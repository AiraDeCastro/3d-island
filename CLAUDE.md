...
- **2026-09-13** — Finished the rest of Milestone 3. Wired the Plausible analytics snippet the user supplied (their account, not something this session can create). With the user's go-ahead, scoped cross-browser QA to Chrome only (no Firefox installed, Safari is impossible on Windows) — full desktop + mobile-emulated pass, all clean. Found and fixed two real things via Lighthouse: (1) `SSAOPass`/`UnrealBloomPass` had no guard against a momentarily zero-sized viewport at startup, causing GL "framebuffer incomplete" warnings until the first resize — added the same clamp `resize()` already had; (2) `SSAOPass` and `GLTFLoader`/`DRACOLoader` were parsed synchronously up front though neither is needed for the first frame — lazy-loaded both (703KB → 637KB main bundle, TBT 2.46s → 1.81s on Lighthouse's mobile simulation). Desktop TTI (1.6s) is comfortably under budget; mobile-simulated TTI (5.4s) is just over the <5s line and left open rather than force-closed — closing it further needs deeper trimming, and deserves a real mid-range-phone number rather than just Lighthouse's pessimistic throttling. Also found the automated browser pane can't sustain a real render loop (rAF effectively stalls without a forced repaint), so live FPS profiling isn't something this environment can do — same class of limitation as the Milestone 2 real-device gap. Fixed a stale `Cache-Control` on production (`vercel.json` now caches hashed `/assets/*` immutably), confirmed on the live Vercel deploy. **Milestone 3 is functionally complete**, with two items intentionally left open (mobile TTI margin, non-Chrome browsers) rather than checked off dishonestly — both documented in `TASKS.md` with what it would take to close them.

- **2026-09-11** — Built most of Milestone 2. With the user's explicit go-ahead, installed Blender 5.2 (winget) headless-only and wrote `tools/blender/build_coconut_tree.py` / `build_beach_hut.py`, which script real geometry and export Draco-compressed `.glb` via Blender's own exporter (~13-16KB each) — replacing the procedural trees from Milestone 1. Wind sway is now injected into standard PBR materials via `onBeforeCompile` (`applyWindSway.ts`) instead of a custom `ShaderMaterial`, reading the sway amount straight from each vertex's local Y so nothing custom needs to survive the glTF round-trip. Added `AudioSystem.ts` (Web Audio-synthesized wind/surf, no audio files — no licensing question, wind volume rides `WindSystem`'s gust envelope), `Controls.ts` + `HintOverlay.ts` (plain DOM, corner-docked mute/quality buttons, idle-fade hint), and `DeviceProfile.ts` (auto-detects a high/low quality tier, driving mesh density at startup and pixel-ratio/bloom/SSAO live). Verified everything on fresh browser tabs (ruling out HMR artifacts that showed up as false-positive console warnings twice this session). One task is genuinely blocked: a real mid-range Android device test — only emulated it, flagged as open in `TASKS.md`. Waiting for the user's go-ahead before Milestone 3.
